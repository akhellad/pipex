...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2023/02/06 18:15:54 by akhellad          #+#    #+#             */
/*   Updated: 2023/02/06 18:16:18 by akhellad         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	child(char **argv, char *envp[], t_pipe *pipex)
{
	...
	if (pipex->pid == 0)
	{
		if (pipex->pipe_pos == 0)
			...
			change_std(pipex->pipe[2 * pipex->pipe_pos - 2], \
						pipex->outfile);
		else
			change_std(pipex->pipe[2 * pipex->pipe_pos - 2], \
					pipex->pipe[2 * pipex->pipe_pos + 1]);
		close_pipes(pipex);
		pipex->arg = ft_split(argv[2 + pipex->pipe_pos], ' ');
